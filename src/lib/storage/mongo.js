...
function getMongoConnectionString() {
    ...
    if (config.mongo.uri_prefix) connectionString += `${config.mongo.uri_prefix}://`;
    ...
    if (config.mongo.cluster_url) connectionString += config.mongo.cluster_url;
    if (config.mongo.mongoDBName) connectionString += `/${config.mongo.mongoDBName}`;
    
    const queryParams = [];
    if (config.mongo.connectTimeoutMS) queryParams.push(`connectTimeoutMS=${config.mongo.connectTimeoutMS}`);
    if (queryParams.length > 0) connectionString += `?${queryParams.join("&")}`;
...
async function updateDeviceStatus(deviceId, status) {
    ...
    const result = await dbHandle.collection(FAMILIES_COLLECTION).updateOne(
